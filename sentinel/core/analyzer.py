...
from sentinel.detectors.trojan import TrojanDetector
...
class MalwareAnalyzer:
    ...
    def _initialize_detectors(self) -> None:
        ...
        # Add new detectors - Trojans and comprehensive detection
        self.detectors.append(TrojanDetector())
        logger.debug("Trojan detector initialized")
        
        self.detectors.append(ComprehensiveMalwareDetector())
        logger.debug("Comprehensive malware detector initialized")
    ...
    def _perform_static_analysis(
        ...
        # Archive analysis for compressed files
        if ext in ['.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz', '.tgz', '.tar.gz']:
            logger.info(f"Detected archive file: {ext}")
            try:
                archive_analyzer = ArchiveAnalyzer()
                static_info['archive_analysis'] = archive_analyzer.analyze(sample_path)
                logger.info("Archive analysis completed")
            except Exception as e:
                logger.error(f"Archive analysis failed: {e}")
                static_info['archive_analysis'] = {'error': str(e)}
        
        # Document analysis for malicious documents
        elif ext in ['.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.pdf', 
                     '.vbs', '.vbe', '.js', '.jse', '.hta', '.ps1', '.psm1', '.psd1',
                     '.rtf', '.dot', '.xlt', '.wsf', '.wsh']:
            logger.info(f"Detected document/script file: {ext}")
            try:
                doc_analyzer = DocumentAnalyzer()
                static_info['document_analysis'] = doc_analyzer.analyze(sample_path)
                logger.info("Document analysis completed")
            except Exception as e:
                logger.error(f"Document analysis failed: {e}")
                static_info['document_analysis'] = {'error': str(e)}
